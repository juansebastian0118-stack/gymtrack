...
function sanitizeCycle(cycle){
  ...
  // Contar slots activos (no reprogramados)
  let activeCount=classes.filter(c=>c.status!=="rescheduled").length;
  if(activeCount<=target) return cycle;
  // Eliminar pendientes sobrantes del final hasta llegar al limite
  const result=[];
  let removed=0;
...
function sanitizeCycles(data){
  return{...data,cycles:data.cycles.map(sanitizeCycle)};
}
...
function cleanData(obj){
  ...
  if(obj && typeof obj === "object"){
    const r={};
    Object.entries(obj).forEach(([k,v])=>{ if(v!==undefined&&v!==null||typeof v==="boolean"||typeof v==="number") r[k]=cleanData(v); });
    return r;
  }
  return obj;
}
      ...
      <div style={{padding:"14px",background:headerBg}}>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginTop:"8px"}}>
          ...
          <div style={{display:"flex",gap:"5px"}}>
            <button onClick={()=>setConfirmDelete(true)} style={{...btnSm,background:C.roseBg,color:"#fb7185",border:`1px solid ${C.roseBd}`}}>🗑️ Eliminar</button>
      ...
      {confirmReset&&(
        <div style={{position:"fixed",inset:0,zIndex:50,background:"rgba(0,0,0,0.8)",backdropFilter:"blur(6px)",display:"flex",alignItems:"center",justifyContent:"center",padding:"16px"}}>
          <div style={{background:C.bg9,border:`1px solid ${C.ambBd}`,borderRadius:"14px",padding:"18px",width:"100%",maxWidth:"370px"}}>
            ...
            <p style={{fontSize:"0.7rem",color:C.z4,marginBottom:"14px",lineHeight:1.6}}>Se regeneran las <strong style={{color:C.z2}}>clases pendientes</strong> con fechas desde hoy.<br/>Las clases realizadas, canceladas y reprogramadas se conservan.</p>
            <div style={{display:"flex",gap:"7px"}}>
      ...
      {confirmDelete&&(
        <div style={{position:"fixed",inset:0,zIndex:50,background:"rgba(0,0,0,0.8)",backdropFilter:"blur(6px)",display:"flex",alignItems:"center",justifyContent:"center",padding:"16px"}}>
          <div style={{background:C.bg9,border:`1px solid ${C.roseBd}`,borderRadius:"14px",padding:"18px",width:"100%",maxWidth:"370px"}}>
            ...
            <p style={{fontSize:"0.7rem",color:C.z4,marginBottom:"14px",lineHeight:1.6}}>Esta acción <strong style={{color:"#fb7185"}}>elimina permanentemente</strong> este ciclo y todas sus clases.<br/>Esta acción no se puede deshacer.</p>
            <div style={{display:"flex",gap:"7px"}}>
...
export default function App(){
  ...
  useEffect(()=>{
    const unsub=onSnapshot(DOC_REF(),snap=>{
      if(snap.exists()){
        const loaded=sanitizeCycles(snap.data().payload);
        setData(loaded);
