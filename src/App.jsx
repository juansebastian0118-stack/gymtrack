...
export default function App(){
  ...
  function rescheduleClass(ci,li,reschd){
    ...
    cls[li]=reschd;
    // Contar slots activos: pending + done + cancelled (NO rescheduled)
    const activeSlots=cls.filter(c=>c.status!=="rescheduled").length;
    // Solo agregar reposicion si faltan slots para llegar al objetivo
    // y no hay ya una clase pendiente de reposicion sin usar
    const pendingMakeups=cls.filter(c=>c.status==="pending"&&c.isMakeup).length;
    if(activeSlots<cycle.config.classesPerCycle&&pendingMakeups===0){
