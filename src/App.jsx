...
async function saveToFirebase(data){try{await setDoc(DOC_REF(),{payload:data,updatedAt:new Date().toISOString()});}catch(e){console.error("Save error:",e);}}
...
function exportCSV(cycles,studentName,teacherName){
  ...
  cycles.forEach((cy,ci)=>{const seqs=buildSeqNums(cy.classes);cy.classes.forEach((c,i)=>rows.push([ci+1,cy.name,studentName,teacherName,seqs[i]??"↺",c.date,c.time,c.status,c.type,c.notes||"",cy.paid?"Pagado":"Pendiente",cy.amount]));});
...
export default function App(){
  ...
  useEffect(()=>{
    const unsub=onSnapshot(DOC_REF(),snap=>{
      if(snap.exists()){setData(snap.data().payload);setLastSaved(snap.data().updatedAt);}
  ...
  const persist=useCallback((nd)=>{
    ...
    saveToFirebase(nd).then(()=>{setSaving(false);setLastSaved(new Date().toISOString());});
  },[]);
