...
// Solo elimina cuando hay estrictamente mas del target — nunca toca clases validas
function sanitizeCycle(cycle){
  ...
  // Eliminar 'excess' clases pendientes del final (preferir las que son isMakeup)
  const classes=[...cycle.classes];
  let removed=0;
  const result=[];
  // Primero intentar eliminar makeups pendientes sobrantes del final
  for(let i=classes.length-1;i>=0&&removed<excess;i--){
    if(classes[i].status==="pending"&&classes[i].isMakeup){
      removed++;
      classes[i]=null;
    }
  }
  // Si aun quedan sobrantes, eliminar pendientes normales del final
  for(let i=classes.length-1;i>=0&&removed<excess;i--){
    if(classes[i]&&classes[i].status==="pending"){
      removed++;
      classes[i]=null;
    }
  }
...
// Default PINs — admin can change them from settings
const DEFAULT_PINS = { admin:"1234", profesor:"5678", alumno:"0000" };

// Storage helpers — graceful fallback if storage unavailable
function loadPins(){
  try{
    const v=sessionStorage.getItem("gymtrack-pins")||localStorage.getItem("gymtrack-pins");
    return v?JSON.parse(v):DEFAULT_PINS;
  }catch{ return DEFAULT_PINS; }
}
function savePins(pins){
  const s=JSON.stringify(pins);
  try{sessionStorage.setItem("gymtrack-pins",s);}catch{}
  try{localStorage.setItem("gymtrack-pins",s);}catch{}
}
function loadSession(){
  try{ return sessionStorage.getItem("gymtrack-role")||null; }
  catch{ return null; }
}
function saveSession(role){
  try{ if(role)sessionStorage.setItem("gymtrack-role",role); else sessionStorage.removeItem("gymtrack-role"); }
  catch{}
}

// Permission helpers
function canEdit(role){ return role==="admin"||role==="profesor"||role==="alumno"; }
...
function LoginScreen({onLogin}){
  ...
  return(
    <div style={{minHeight:"100vh",background:"#09090b",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'DM Sans',system-ui,sans-serif",padding:"20px"}}>
      ...
      <div style={{width:"100%",maxWidth:"360px"}}>
        ...
        <div style={{textAlign:"center",marginBottom:"32px"}}>
          <div style={{width:"56px",height:"56px",borderRadius:"16px",background:"linear-gradient(135deg,#0ea5e9,#7c3aed)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"1.5rem",fontWeight:800,margin:"0 auto 12px"}}>G</div>
          <div style={{fontSize:"1.4rem",fontWeight:800,color:"#f4f4f5"}}>GymTrack</div>
          <div style={{fontSize:"0.75rem",color:"#71717a",marginTop:"4px"}}>Yony Vega × Juan Sebastian</div>
        </div>

        {!selectedRole ? (
          /* Role selection */
          <div>
            <div style={{fontSize:"0.85rem",color:"#a1a1aa",textAlign:"center",marginBottom:"16px"}}>Selecciona tu perfil</div>
            <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
              {Object.entries(ROLES).map(([key,r])=>(
                <button key={key} onClick={()=>{setSelectedRole(key);setPin("");setError("");}} style={{
                  padding:"14px 20px",borderRadius:"12px",border:"1px solid #3f3f46",
                  background:"#18181b",color:"#e4e4e7",fontSize:"0.95rem",fontWeight:600,
          ...
          /* PIN entry */
          <div>
            <button onClick={()=>{setSelectedRole(null);setPin("");setError("");}} style={{background:"none",border:"none",color:"#71717a",fontSize:"0.78rem",cursor:"pointer",marginBottom:"20px",display:"flex",alignItems:"center",gap:"4px"}}>← Cambiar perfil</button>
            <div style={{textAlign:"center",marginBottom:"24px"}}>
              <div style={{display:"inline-flex",alignItems:"center",gap:"8px",padding:"6px 14px",borderRadius:"999px",background:"#18181b",border:"1px solid #3f3f46",marginBottom:"16px"}}>
                <span style={{width:"8px",height:"8px",borderRadius:"50%",background:ROLES[selectedRole].color}}/>
                <span style={{fontSize:"0.8rem",color:"#e4e4e7",fontWeight:600}}>{ROLES[selectedRole].label}</span>
              </div>
              <div style={{fontSize:"0.85rem",color:"#a1a1aa",marginBottom:"20px"}}>Ingresa tu PIN</div>
              {/* PIN dots */}
              <div style={{display:"flex",justifyContent:"center",gap:"14px",marginBottom:"8px"}}>
                {[0,1,2,3].map(i=>(
                  <div key={i} style={{width:"14px",height:"14px",borderRadius:"50%",background:i<pin.length?ROLES[selectedRole].color:"#3f3f46",transition:"background 0.15s"}}/>
                ))}
              </div>
              {error&&<div style={{fontSize:"0.72rem",color:"#fb7185",marginTop:"8px"}}>{error}</div>}
            </div>
            {/* Numpad */}
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"10px"}}>
              {[1,2,3,4,5,6,7,8,9,"",0,"⌫"].map((d,i)=>(
                <button key={i} onClick={()=>d===""?null:d==="⌫"?handleBack():handleDigit(String(d))}
                  disabled={d===""}
                  style={{
                    padding:"16px",borderRadius:"12px",fontSize:"1.1rem",fontWeight:700,cursor:d===""?"default":"pointer",
                    border:"1px solid #3f3f46",background:d===""?"transparent":"#18181b",
                    color:d==="⌫"?"#a1a1aa":"#e4e4e7",transition:"all 0.1s",
                    opacity:d===""?0:1
                  }}>
                  {d}
                </button>
...
function ChangePinsModal({onClose}){
  ...
  function save(){
    // Validate: all PINs must be 4 digits
    for(const[role,pin] of Object.entries(pins)){
      if(!/^\d{4}$/.test(pin)){
        alert(`El PIN de ${ROLES[role].label} debe ser exactamente 4 dígitos.`);
        return;
      }
    }
    savePins(pins);setSaved(true);
    setTimeout(()=>{setSaved(false);onClose();},800);
  }
  return(
    <div style={{position:"fixed",inset:0,zIndex:60,background:"rgba(0,0,0,0.8)",backdropFilter:"blur(6px)",display:"flex",alignItems:"center",justifyContent:"center",padding:"16px"}}>
      <div style={{background:"#18181b",border:"1px solid #3f3f46",borderRadius:"16px",padding:"20px",width:"100%",maxWidth:"360px"}}>
        <div style={{fontSize:"0.95rem",fontWeight:700,marginBottom:"16px",color:"#f4f4f5"}}>🔐 Cambiar PINs</div>
        {Object.entries(ROLES).map(([key,r])=>(
          <div key={key} style={{marginBottom:"12px"}}>
            <label style={{fontSize:"0.7rem",color:"#a1a1aa",display:"flex",alignItems:"center",gap:"6px",marginBottom:"5px"}}>
              <span style={{width:"8px",height:"8px",borderRadius:"50%",background:r.color,flexShrink:0}}/>
              {r.label}
            </label>
            <input type="password" maxLength={4} value={pins[key]}
              onChange={e=>setPins({...pins,[key]:e.target.value.replace(/\D/g,"").slice(0,4)})}
              placeholder="4 dígitos"
              style={{width:"100%",background:"#09090b",border:"1px solid #3f3f46",borderRadius:"8px",padding:"8px 12px",color:"#e4e4e7",fontSize:"1rem",outline:"none",letterSpacing:"0.3em",boxSizing:"border-box"}}/>
          </div>
        ))}
        <div style={{display:"flex",gap:"8px",marginTop:"16px"}}>
          <button onClick={save} style={{flex:1,background:saved?"rgba(5,150,105,0.7)":"#0284c7",border:"none",color:"white",padding:"8px",borderRadius:"10px",fontSize:"0.875rem",cursor:"pointer",fontWeight:700}}>
            {saved?"✓ Guardado":"Guardar PINs"}
          </button>
          <button onClick={onClose} style={{flex:1,background:"#3f3f46",border:"none",color:"#d4d4d8",padding:"8px",borderRadius:"10px",fontSize:"0.875rem",cursor:"pointer"}}>Cancelar</button>
        </div>
      </div>
    </div>
  );
...
function CycleCard({cycle,cycleIndex,isCurrent,role,onUpdateCycle,onUpdateClass,onUpdateClassRecalc,onCancelClass,onRescheduleClass,onResetCycle,onDeleteCycle}){
  ...
  // Alumno no puede gestionar ciclos
  const canManageCycle=canManage(role);
  // Alumno no puede editar config si hay clases registradas
      ...
      <div style={{padding:"14px",background:headerBg}}>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginTop:"8px"}}>
          ...
          <div style={{display:"flex",gap:"5px"}}>
            {/* Eliminar — solo admin/profesor y todas pendientes */}
            {canManageCycle&&<button
            ...
            {/* Reiniciar — solo admin/profesor y todas pendientes */}
            {canManageCycle&&<button
            ...
            {canEditConfig&&<button onClick={()=>setEditingConfig(!editingConfig)} style={btnSm}>⚙️ Config</button>}
...
export default function App(){
  ...
  const[role,setRole]=useState(()=>{ try{return sessionStorage.getItem("gymtrack-role")||null;}catch{return null;} });
  const[showPins,setShowPins]=useState(false);
  const isFirstLoad=useRef(true);

  // Show login if no role
  if(!role) return <LoginScreen onLogin={r=>{ saveSession(r); setRole(r); }}/>;
  ...
  function rescheduleClass(ci,li,reschd){
    ...
    cls[li]=reschd;
    // Recalcular slots activos DESPUES de marcar (pending+done+cancelled)
    const activeSlots=cls.filter(c=>c.status!=="rescheduled").length;
    // Agregar reposicion si los slots activos son menos que el objetivo
    if(activeSlots<cycle.config.classesPerCycle){
        ...
        <div style={{maxWidth:"680px",margin:"0 auto",padding:"9px 14px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div style={{display:"flex",alignItems:"center",gap:"9px"}}>
            ...
            <div>
              ...
              <div style={{fontSize:"0.65rem",lineHeight:1,marginTop:"2px",display:"flex",alignItems:"center",gap:"5px"}}>
                <span style={{color:C.z5}}>{data.teacherName} × {data.studentName}</span>
                <span style={{padding:"1px 6px",borderRadius:"999px",fontSize:"0.58rem",fontWeight:700,background:ROLES[role]?.color+"22",color:ROLES[role]?.color,border:`1px solid ${ROLES[role]?.color}44`}}>{ROLES[role]?.label}</span>
              </div>
            </div>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:"5px"}}>
            ...
            {canManage(role)&&<button onClick={()=>{setLStudent(data.studentName);setLTeacher(data.teacherName);setEditNames(true);}} style={btnA}>✏️ Nombres</button>}
            {canManage(role)&&<button onClick={()=>exportCSV(data.cycles,data.studentName,data.teacherName)} style={btnA}>⬇️ Excel</button>}
            {isAdmin(role)&&<button onClick={()=>setShowPins(true)} style={btnA}>🔐 PINs</button>}
            <button onClick={()=>{saveSession(null);setRole(null);}} style={{...btnA,color:"#fb7185"}} title="Cerrar sesión">⏏️</button>
          </div>
        </div>
      </div>
        ...
        {tab==="history"&&(
          <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
            ...
            {data.cycles.map((cycle,ci)=>{
              ...
              const resc=rescheduledCount(cycle.classes);
              ...
              return(
                <div key={cycle.id} style={{background:"rgba(24,24,27,0.7)",border:`1px solid ${C.bg7}`,borderRadius:"11px",padding:"14px"}}>
                  ...
                  {/* Fila 1: conteos de estado */}
                  <div style={{display:"flex",flexWrap:"wrap",gap:"5px",marginBottom:"8px"}}>
                    {[
                      {v:done, l:"Realizadas",  c:C.em,   bg:C.emBg,  bd:C.emBd},
                      {v:resc, l:"Reprogramadas",c:C.ora, bg:C.oraBg, bd:C.oraBd},
                      {v:canc, l:"Canceladas",  c:"#fb7185",bg:C.roseBg,bd:C.roseBd},
                      {v:pend, l:"Pendientes",  c:C.amb,  bg:C.ambBg, bd:C.ambBd},
                    ].map((s,i)=>(
                  ...
                  {/* Fila 2: presenciales vs virtuales */}
