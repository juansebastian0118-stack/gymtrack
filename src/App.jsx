...
export default function App(){
  ...
  useEffect(()=>{
    const unsub=onSnapshot(SYSTEM_REF(),snap=>{
      if(snap.exists()){
        ...
        // First time: create default system config
        setSystemConfig(DEFAULT_SYSTEM);
        saveSystemConfig(DEFAULT_SYSTEM);
      }
    },err=>{console.error("System config error:",err);setSystemConfig(DEFAULT_SYSTEM);});
    return()=>unsub();
  },[]);
  ...
  },[session,systemConfig,selectedStudentId]);

  if(!session||!systemConfig){
